...
import { SearchFilters, SearchParams } from "@/components/paginas/SearchFilters";
import { FAIXAS_IDADE } from "@/lib/constants";
import { ExternalLink, Globe, Calendar, Server, Tag, FileText, AlertCircle, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
...
export default function MineradorSearchPage() {
  ...
  // Controle de Paginação via Cursor (Search After)
  const [currentPage, setCurrentPage] = useState(1);
  const [cursorHistory, setCursorHistory] = useState<(any[] | null)[]>([null]); // stack de cursors
  ...
  const executeSearch = async (params: SearchParams, page: number, cursor: any[] | null) => {
    ...
    // Para UX, mantemos os resultados anteriores visíveis (mas com opacidade) 
    // enquanto carregamos a nova página, a menos que seja uma busca totalmente nova.
    ...
    try {
      ...
      setResultados(res.resultados as any);
      setTotal(res.total);
      ...
      // Armazena o token do FINAL desta página para a PRÓXIMA página
      const newHistory = [...cursorHistory];
      newHistory[page] = res.lastSortToken; // lastSortToken é o cursor para a próxima página
  ...
  const handleNextPage = () => {
    if (lastParams && !loading && hasMore) {
      const nextCursor = cursorHistory[currentPage]; // este foi salvo durante o load da página atual
  ...
  return (
    <div className="flex flex-col gap-6 p-6 min-h-screen">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold tracking-tight">Minerador de Domínios</h1>
      ...
      {resultados.length > 0 && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm text-muted-foreground">
            ...
            <code className="hidden xl:block text-[10px] bg-muted/50 px-2 py-1 rounded border border-border max-w-[300px] truncate">{currentQuery}</code>
          </div>
          ...
          <div className="relative min-h-[400px]">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-opacity duration-300 ${loading ? 'opacity-30' : 'opacity-100'}`}>
              {resultados.map((item, i) => (
                <div key={item.dominio + i} className="group relative flex flex-col justify-between overflow-hidden rounded-xl border border-border bg-card shadow transition-all hover:shadow-md hover:border-primary/50">
                  {/* Screenshots */}
                  ...
                  <div className="p-4 flex flex-col gap-3 flex-1">
                    ...
                    <div className="grid grid-cols-1 gap-2 text-[11px] text-muted-foreground">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate">{item.pais || "???"} {item.ip ? `(${item.ip})` : ""}</span>
                      </div>
                      <div className="flex items-center gap-2">
